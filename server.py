...
# The public site, login, booking, invoice and payment handoff use one canonical
# customer-facing origin. This avoids splitting the reviewed merchant journey
# across different website hosts.
FRONTEND_URL = os.environ.get("FRONTEND_URL", "https://www.ptrconnect.online").strip().rstrip("/")
PAYWAY_RETURN_URL = os.environ.get(
    "PAYWAY_RETURN_URL",
    "https://ptr-connect-api.onrender.com/api/payments/payway/callback",
).strip()
...
def create_payway_payment_link(invoice, booking, merchant_ref_no):
    ...
    merchant_data = {
        ...
        "title": f"PRATHNA KASEKOR KHMER IMPORT EXPORT CO., LTD. invoice {invoice.reference}"[:250],
