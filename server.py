...
class HostedCheckout(db.Model):
    """Server-owned state for the ABA eCommerce Purchase/hosted-checkout flow."""
    id = db.Column(db.Integer, primary_key=True)
    booking_id = db.Column(db.String(40), unique=True, nullable=False, index=True)
    request_id = db.Column(db.String(128), unique=True, nullable=False, index=True)
    tran_id = db.Column(db.String(32), unique=True, nullable=False, index=True)
    package_id = db.Column(db.String(100), nullable=False)
    guest_name = db.Column(db.String(200), nullable=False)
    amount = db.Column(db.Numeric(12, 2), nullable=False)
    currency = db.Column(db.String(3), default="USD", nullable=False)
    status = db.Column(db.String(40), default="PAYMENT_PENDING", nullable=False)
    apv = db.Column(db.String(100))
    created_at = db.Column(db.DateTime(timezone=True), default=now, nullable=False)
    updated_at = db.Column(db.DateTime(timezone=True), default=now, onupdate=now, nullable=False)
...
def payway_purchase_hash(fields, api_key):
    # PayWay requires the documented purchase-field order. Optional fields that
    # are not posted are omitted from the signed message.
    message = "".join(str(fields[key]) for key in PAYWAY_PURCHASE_HASH_ORDER if key in fields)
    return payway_hmac(message, api_key)
...
def hosted_checkout_amount(data):
    ...
    if not service:
        raise ValueError("Unsupported booking service")
    guest_label = str(data.get("guests") or "1")
    guest_match = re.search(r"\d+", guest_label)
    quantity = int(guest_match.group(0)) if guest_match else 1
    quantity = max(1, min(quantity, MAX_BOOKING_QUANTITY))
    nights = 1
    if service["category"] in {"hotel", "villa"}:
        try:
            start = datetime.fromisoformat(str(data.get("checkIn") or "")).date()
            end = datetime.fromisoformat(str(data.get("checkOut") or "")).date()
        except (TypeError, ValueError):
            raise ValueError("Invalid booking dates")
        nights = (end - start).days
        if not 1 <= nights <= 365:
            raise ValueError("Check-out must be after check-in")
    amount = money(service["unit_price"] * quantity * nights)
    validate_payway_total(amount)
    return package_id, service, quantity, nights, amount
...
@app.post("/api/payway/purchase")
def create_hosted_purchase():
    limited = _client_ip_rate_limit("hosted-purchase", PAYMENT_LINK_RATE_LIMIT)
    if limited:
        return limited
    data = request.get_json(silent=True)
    if not isinstance(data, dict):
        return jsonify(message="A JSON booking request is required."), 400
    ...
    if existing:
        if existing.status in {"PAID", "CONFIRMED", "CANCELLED"}:
            return jsonify(bookingId=existing.booking_id, status=existing.status), 200
        return jsonify(message="This payment session has already been created.", bookingId=existing.booking_id), 409
    guest_name = str(data.get("guestName") or "").strip()
    if not guest_name or len(guest_name) > 200:
        return jsonify(message="Guest name is required."), 400
    try:
        package_id, service, quantity, nights, amount = hosted_checkout_amount(data)
    except (InvalidOperation, TypeError, ValueError) as exc:
        return jsonify(message=str(exc) or "Invalid booking amount."), 400
    if not payway_is_configured():
        return jsonify(message="Payment service configuration is incomplete. No payment was attempted."), 503

    config = payway_config()
    tran_id = f"{int(time.time() * 1000)}{secrets.randbelow(10000):04d}"
    booking_id = "PKK-" + secrets.token_hex(8).upper()
    name_parts = re.sub(r"[^\w\s'-]", "", guest_name, flags=re.UNICODE).split()
    firstname = (name_parts[0] if name_parts else "Guest")[:20]
    lastname = (" ".join(name_parts[1:]) if len(name_parts) > 1 else "Customer")[:20]
    item_payload = [{
        "name": service["name"], "quantity": quantity,
        "price": f"{money(service['unit_price'] * nights):.2f}",
    }]
    fields = {
        "req_time": payway_request_time(),
        "merchant_id": config["merchant_id"],
        "tran_id": tran_id,
        "amount": f"{amount:.2f}",
        "items": base64.b64encode(json.dumps(item_payload, separators=(",", ":")).encode("utf-8")).decode("ascii"),
        "firstname": firstname,
        "lastname": lastname,
        "type": "purchase",
    ...
    fields["hash"] = payway_purchase_hash(fields, config["api_key"])
    checkout = HostedCheckout(
        booking_id=booking_id, request_id=request_id, tran_id=tran_id,
        package_id=package_id, guest_name=guest_name, amount=amount,
        currency="USD", status="PAYMENT_PENDING",
    )
    ...
    try:
        ...
        db.session.rollback()
        return jsonify(message="This payment session is already being processed."), 409
    app.logger.info("PayWay hosted checkout prepared booking=%s mode=%s", booking_id, payway_mode())
    return jsonify(
        bookingId=booking_id, status=checkout.status,
        action=config["base_url"] + PAYWAY_PURCHASE_PATH, fields=fields,
    ), 201


def reconcile_hosted_checkout(checkout):
    result = check_payway_transaction(checkout.tran_id)
    status = result.get("status") if isinstance(result.get("status"), dict) else {}
    data = result.get("data") if isinstance(result.get("data"), dict) else {}
    approved = str(status.get("code") or "") == "00" and int(data.get("payment_status_code", -1)) == 0
    if approved:
        try:
            checked_amount = money(data.get("payment_amount"))
        except (InvalidOperation, TypeError, ValueError):
            checked_amount = Decimal("-1.00")
        if checked_amount != money(checkout.amount) or str(data.get("payment_currency") or "").upper() != checkout.currency:
            checkout.status = "REVIEW_REQUIRED"
        else:
            checkout.status = "CONFIRMED"
            checkout.apv = str(data.get("apv") or "")[:100]
    elif int(data.get("payment_status_code", 2)) in {1, 3, 4, 11}:
...
@app.get("/api/payway/status")
def hosted_purchase_status():
    limited = _client_ip_rate_limit("hosted-status", PAYMENT_STATUS_RATE_LIMIT)
    if limited:
        return limited
    booking_id = request.args.get("bookingId", "").strip()
    checkout = HostedCheckout.query.filter_by(booking_id=booking_id).first()
    if not checkout:
        return jsonify(message="Payment session was not found."), 404
    if checkout.status == "PAYMENT_PENDING":
        try:
            reconcile_hosted_checkout(checkout)
        except PayWayError:
            db.session.rollback()
    return jsonify(bookingId=checkout.booking_id, status=checkout.status,
                   message="Payment status checked securely with ABA PayWay."), 200


@app.post("/api/payway/cancel")
def cancel_hosted_purchase():
    data = request.get_json(silent=True) or {}
    checkout = HostedCheckout.query.filter_by(booking_id=str(data.get("bookingId") or "").strip()).first()
    if not checkout:
        return jsonify(message="Payment session was not found."), 404
    if checkout.status == "CONFIRMED":
        return jsonify(message="A confirmed payment cannot be cancelled here."), 409
    checkout.status = "CANCELLED"
    db.session.commit()
    return jsonify(bookingId=checkout.booking_id, status=checkout.status), 200
...
def payway_payment_callback():
    ...
    # ABA eCommerce Purchase pushback: verify the signed JSON first, then use
    # Check Transaction as the authoritative payment decision.
    purchase_booking_id = str(payload.get("return_params") or "").strip()
    purchase_tran_id = str(payload.get("tran_id") or "").strip()
    if purchase_booking_id:
        ...
        try:
            if not verify_payway_callback_signature(payload, received_signature):
                return jsonify(error="Invalid callback signature"), 401
        except PayWayError:
            return jsonify(error="Payment verification is unavailable"), 503
        checkout = HostedCheckout.query.filter_by(
            booking_id=purchase_booking_id, tran_id=purchase_tran_id
        ).first()
        if not checkout:
        ...
        if str(payload.get("status") or "") != "0":
            checkout.status = "PAYMENT_FAILED"
            db.session.commit()
            return jsonify(received=True, paid=False), 200
        try:
            status = reconcile_hosted_checkout(checkout)
        except PayWayError:
            db.session.rollback()
            return jsonify(received=True, verification="deferred"), 202
        return jsonify(received=True, paid=status == "CONFIRMED"), 200
